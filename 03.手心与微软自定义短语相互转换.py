...
def Gen(table: Table) -> bytes:
    buf = io.BytesIO()
    stamp = int(time.time())
    # 搜狗输入法的文件头可能不同，需要根据实际文件调整
    buf.write(b"sogou\x02\x00`\x00\x01\x00\x00\x00")  # 修改文件头为搜狗格式
...
# ---- 主程序 ----
def main():
    print("欢迎使用搜狗与微软自定义短语转换工具\n====！！！！！请注意输入输出时的txt均为utf8编码，请注意转换！！！！=====")
    print("1. dat 转 txt")
    print("2. txt 转 dat")
    choice = input("请选择操作 (1 或 2): ")

    if choice == "1":
        input_file = input("请输入 .dat 文件路径 (默认为 搜狗.dat): ").strip() or "搜狗.dat"
        output_file = input("请输入输出的 .txt 文件路径 (默认为 搜狗.txt): ").strip() or "搜狗.txt"
        ...
        input_file = input("请输入 .txt 文件路径 (默认为 搜狗.txt): ").strip() or "搜狗.txt"
        output_file = input("请输入输出的 .dat 文件路径 (默认为 搜狗.dat): ").strip() or "搜狗.dat"
        if not os.path.exists(input_file):
            print(f"错误：文件 {input_file} 不存在！")
            return
        table = []
        with open(input_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                code, order = code_order.split("=")
                table.append(Entry(word, code, int(order)))  # 添加权重字段
